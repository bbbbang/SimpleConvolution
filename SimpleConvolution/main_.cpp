...
int main()
{
	...
	//while (!capture.isOpened())
	{
		...
		for (int i = 0; i < 10; ++i)
		{
			if (result[i].score > 0.5f)
				cv::rectangle(frame, cv::Point(result[i].x1*720/160, result[i].y1 * 720 / 160), cv::Point(result[i].x2 * 720 / 160, result[i].y2 * 720 / 160), cv::Scalar(255, 0, 0));
		}
	...
	*/
	...
	//	//for (int i = 0; i < 10; ++i)
	//	//{
	//	//	std::cout << temp[i].category << ", " << temp[i].score << "  (" << temp[i].x1 << ", " << temp[i].y1 << ", " <<  temp[i].x2 << ", " << temp[i].y2 << ")" << std::endl;
	//	//}
